...
class RenameTool(Rename):
	...
	def Rename_UI(self, processCall = 'NA'):
		...
		# add functions
		cmds.textField('inputField', edit = True, alwaysInvokeEnterCommandOnReturn = True, enterCommand = partial (self.process, processCall) )
		cmds.textField("replaceField", edit = True, alwaysInvokeEnterCommandOnReturn = True, enterCommand = partial (self.process, processCall) )
		cmds.textField("withField", edit = True, alwaysInvokeEnterCommandOnReturn = True, enterCommand = partial (self.process, processCall) )
		cmds.setFocus('withField') # set focus to the with field so it's easier to just tab to the input field
		cmds.showWindow(window)
	####
	def process( self, call, *args):
		# Selection
		...
		# Assign the properties the Rename Class needs
		self.name = inputName
		self.selection = selections + inputName
		#D = Rename(selections, inputName )
		if old: # if there is data in the replaceField txt box, then run the replacement code
			for i in range( len (selections) ):
				findTxt = self.reFunction(self.getAfterNamespace(i), old)
				replacementName = self.getAfterNamespace(i)[ : findTxt [0][0] ] + new + self.getAfterNamespace(i)[ findTxt [0][1] : ]
				cmds.rename(selections[i], self.getNamespace(i) + replacementName)
		else:
			for i in range( len (selections) ):
				X = self.processNamespace(i)
				if X == ':': X = '::' # This prevents the root namespace from getting lost in the cutoff [:-1] use X[:-1] instead of self.processNamespace(i) [:-1]
				...
				cmds.rename( self.selection[i] , ':' + self.processNamespace(i) + self.processRename(i) )
